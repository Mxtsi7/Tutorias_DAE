-- LoginScreen.lua  (redise\xc3\xb1o sin autenticaci\xc3\xb3n)
-- Paso 1: elegir actor (Estudiante / Tutor / Coordinador)
-- Paso 2: lista de usuarios de ese rol + crear uno nuevo con solo nombre
-- Al seleccionar un usuario la app inicia sesi\xc3\xb3n y navega al dashboard.
...
-- ----------------------------------------------------------------
-- PASO 1: dibuja la pantalla de elecci\xc3\xb3n de actor
-- ----------------------------------------------------------------
local function drawPaso1()
    ...
    -- Footer  (sin punto medio Unicode, solo ASCII)
    love.graphics.setColor(Colors.textSub)
    love.graphics.setFont(Fonts.small)
    love.graphics.printf("Modo demo  |  Sin autenticacion  |  DAE 2026",
        0, HH-28, WW, "center")
end

-- ----------------------------------------------------------------
-- PASO 2: dibuja la lista de usuarios + formulario de creaci\xc3\xb3n
-- ----------------------------------------------------------------
local function drawPaso2()
    ...
    if #usuarios == 0 then
        ...
        for i, u in ipairs(usuarios) do
            ...
            -- ID e info: sin \xc2\xb7, usar " | " ASCII
            love.graphics.setColor(Colors.textSub[1], Colors.textSub[2], Colors.textSub[3], alpha)
            love.graphics.setFont(Fonts.small)
            love.graphics.print("ID #" .. tostring(u.id) ..
                " | " .. (u.rol or rolActivo or ""),
                MARGIN+52, ry+28)
    ...
    -- Boton Volver  (sin flecha Unicode, usar "< Volver" ASCII)
    local btnY = HH - 78
...
function LS.mousepressed(x, y, btn)
    ...
    if paso == 1 then
        ...
        local formY = math.max(140 + #usuarios*58 + 20, math.floor(HH * 0.55))
        local fieldX = MARGIN + 16
        local fieldY = formY + 42
        local fieldW = listW - 32 - 180
        local fieldH = 36
