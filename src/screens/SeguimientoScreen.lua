...
-- -------------------------------------------------------
-- MODAL
-- Muestra SIEMPRE al menos un boton de accion relevante
-- segun el estado actual de la tutoria.
-- -------------------------------------------------------
local function drawModal(t)
    ...
    local btnH = 38
    local gap  = 14
    local bpos = btnX   -- posicion acumulada de siguiente boton
    ...
    if ESTADOS_FINALES[t.estado] then
        -- Solo mostrar etiqueta informativa, ningun boton extra
        love.graphics.setColor(Colors.textSub)
        love.graphics.setFont(Fonts.small)
    ...
    elseif t.estado == "activa_con_alerta" then
        -- 1ra ausencia: Advertir o Suspender
    ...
    elseif t.estado == "suspendida" then
        -- Coordindaor puede cerrar por abandono
        love.graphics.setColor(Colors.red)
        ...
        -- activa, activa_con_advertencia, etc: proponer cierre exitoso
        love.graphics.setColor(Colors.green)
        love.graphics.rectangle("fill", bpos, btnY, btnW+20, btnH, 10)
        love.graphics.setColor(1,1,1)
        love.graphics.setFont(Fonts.body)
...
function Seg.mousepressed(x, y, btn)
    ...
    if detalle then
        ...
        -- Decision formal (advertencia_formal o >=2 ausencias)
        ...
        -- Proponer cierre exitoso (activa o activa_con_advertencia)
